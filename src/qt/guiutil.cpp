...
{
    // JiaguCoin: check prefix
    if(uri.scheme() != QString("JiaguCoin"))
        return false;
...
{
    // Convert JiaguCoin:// to JiaguCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("JiaguCoin://"))
    {
        uri.replace(0, 12, "JiaguCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "JiaguCoin.lnk";
}
...
{
    return GetAutostartDir() / "JiaguCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("JiaguCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  JiaguCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("JiaguCoin-Qt"));
